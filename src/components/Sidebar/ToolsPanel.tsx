...
export function ToolsPanel({ isSmallWidth }: ToolsPanelProps) {
    const { settings, document, closeDocument, setSidebarCollapse, setActiveTool, setCurrentTextStyle, setBrushSettings, updateAnnotation, selectAnnotation } = usePDFStore();
    const { activeTool, currentTextStyle, brushSettings, selectedAnnotationId, selectedAnnotationPageId } = settings;
    ...
    return (
        <div className="py-3 space-y-3 mb-10">
            {showColorPicker && (
                <div className="mx-3">
                    ...
                    <label className="flex items-center w-full h-9 rounded-lg border border-(--color-border) cursor-pointer overflow-hidden transition-all hover:border-primary relative">
                        ...
                        <input type="color" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" value={getActiveToolColor()} onChange={(e) => handleToolColorChange(e.target.value)} />
                    </label>
                </div>
            )}
                ...
                <div className="mx-3">
                    <div className="bg-(--color-bg-app) border border-(--color-border) rounded-xl p-3 space-y-2.5">
                        ...
                        <div className="flex gap-1">
                            <button className={`flex-1 p-1.5 rounded-lg transition-all flex items-center justify-center ${!currentFontDef.supportsBold ? 'opacity-30 cursor-not-allowed' : activeStyle.bold ? 'bg-primary text-white' : 'bg-(--color-bg-panel) text-(--color-text-muted) border border-(--color-border)'}`}
                            ...
                            ><Bold size={14} /></button>
                            <button className={`flex-1 p-1.5 rounded-lg transition-all flex items-center justify-center ${!currentFontDef.supportsItalic ? 'opacity-30 cursor-not-allowed' : activeStyle.italic ? 'bg-primary text-white' : 'bg-(--color-bg-panel) text-(--color-text-muted) border border-(--color-border)'}`}
            ...
            <ConfirmationModal
                ...
                onConfirm={() => { closeDocument(); setShowExitConfirm(false); }}
